...
use super::{Item, ReadPipelineImpl, UniversalReadFs, UserData};
...
pub trait UniversalRead: Sized + Debug + Send + Sync {
    ...
    /// [`OwningPipeline<Self::ReadPipeline<'static, U>, Self, U>`](super::OwningPipeline);
    /// backends do not implement it separately.
    type ReadPipeline<'file, U>: ReadPipelineImpl<'file, U, File = Self>
    where
        Self: 'file,
        U: UserData;
